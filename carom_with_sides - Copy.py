...
sq3=math.sqrt(3)
line_r=60
#boardimg=PhotoImage(file="newb.gif")
boardimg=ImageTk.PhotoImage(Image.open("smoothboard.jpg").resize((1200,1200),Image.ANTIALIAS))
boardimg=ImageTk.PhotoImage(Image.open("board.png").resize((1150,1150),Image.ANTIALIAS))
reddiskimg=ImageTk.PhotoImage(Image.open("red_disk.png").resize((50,50),Image.ANTIALIAS))
blackdisk=ImageTk.PhotoImage(Image.open("black_disk.png").resize((50,50),Image.ANTIALIAS))
whitedisk=ImageTk.PhotoImage(Image.open("white_disk.png").resize((50,50),Image.ANTIALIAS))
partner="partner_1"
...
def scale_disc(event):
    if event.x>line_r+R+c and event.x<w-line_r-R-c and partner=="partner_1":
        canvas_scale.delete("scale_disc")
        canvas_scale.create_oval(event.x-R,15-R,event.x+R,15+R,fill="black",width=0,tag="scale_disc")
...
def set_maindisc_velocity(event):
   ...
   # canvas.create_line(event.x,event.y,click_x,click_y,fill="yellow",tag="vline",width="2",dash=(200,1))
    canvas.create_oval(click_x-l,click_y-l,click_x+l,click_y+l,outline="yellow",tag="vline",width="2",dash=(50,2))
        ...
        for b1 in list_ball:
            if not(b1["vx"]==0 and b1["vy"]==0):
                ...
                if b1["color"]==1:
                    ...
                    canvas_m.create_oval(w-b1["x"]-r,h-b1["y"]-r,w-b1["x"]+r,h-b1["y"]+r,fill="#692000000", width="0",tag=b1["id"],outline=None)
...
canvas_scale=Canvas(root,width=w,height="30",bg="brown")
canvas_scale.grid(row=2,column=0)
canvas_scale_m=Canvas(root,width=w,height="30",bg="brown")
canvas_scale_m.grid(row=2,column=2)
canvas_scale.create_oval(w/2-R,15-R,w/2+R,15+R,fill="black",tag="scale_disc")
canvas_scale.create_oval(w/2-R2,15-R2,w/2+R2,15+R2,fill="white",tag="scale_disc")
...
for b in list_ball:
    if b["color"]==1:
        ...
        canvas_m.create_oval(w-b["x"]-r,h-b["y"]-r,w-b["x"]+r,h-b["y"]+r,fill="#692000000", width="0",tag=b["id"],outline=None)
        canvas_m.create_oval(w-b["x"]-r2,h-b["y"]-r2,w-b["x"]+r2,h-b["y"]+r2,fill="#a18000000",width="0", tag=b["id"],outline="#a18000000")
        canvas_m.create_oval(w-b["x"]-r3,h-b["y"]-r3,w-b["x"]+r3,h-b["y"]+r3,fill="#edb000000", width="0",tag=b["id"],outline="#edb000000")
    else:
        ...
        canvas_m.create_oval(w-b["x"]-r2,h-b["y"]-r2,w-b["x"]+r2,h-b["y"]+r2,fill="#00030c418",width="0", tag=b["id"],outline="#a18000000")
